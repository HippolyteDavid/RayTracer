...
Math::Vector3D LiveRenderer::reflectRay(
    Math::Vector3D normalVector, Math::Vector3D ray)
{
    return (normalVector * normalVector.dot(ray) * 2) - ray;
}

Color LiveRenderer::computeLight(Math::Point3D intersectionPoint,
...
intersection LiveRenderer::getClosestIntersection(
...
void LiveRenderer::computeScene()
{
    for (; this->y > -(this->scene_.getCamera().imageSize_.height / 2);
        ...
        for (; this->x < (this->scene_.getCamera().imageSize_.width / 2);
            ...
            this->result_[this->index].emplace_back(this->traceRay(
                { this->scene_.getCamera().origin, D }, 1, DBL_MAX, 3));
            if (isCommand()) {
                this->x++;
                return;
            }
...
void LiveRenderer::initIterators()
{
    this->done_ = false;
    this->index = 0;
...
bool LiveRenderer::isGenerationDone() { return this->done_; }

Scene& LiveRenderer::getScene() { return this->scene_; }

std::vector<std::vector<Color>> LiveRenderer::getResult()
{
    return this->result_;
}

bool LiveRenderer::canWrite() { return this->canWrite_; }
}
