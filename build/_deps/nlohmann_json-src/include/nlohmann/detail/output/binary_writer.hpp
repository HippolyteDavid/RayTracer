  ...
  public:
    ...
    void write_bson(const BasicJsonType& j)
    {
        ...
        default: {
            JSON_THROW(type_error::create(317,
                concat("to serialize to BSON, top-level type must be object, "
                       "but is ",
                    j.type_name()),
                &j));
        }
    ...
    */
        ...
        const bool use_bjdata = false)
    {
        ...
        case value_t::number_unsigned: {
            write_number_with_ubjson_prefix(
                j.m_value.number_unsigned, add_prefix, use_bjdata);
            break;
        }

        case value_t::number_float: {
            write_number_with_ubjson_prefix(
                j.m_value.number_float, add_prefix, use_bjdata);
            break;
        }
        ...
        case value_t::string: {
            ...
            write_number_with_ubjson_prefix(
                j.m_value.string->size(), true, use_bjdata);
            oa->write_characters(
                reinterpret_cast<const CharType*>(j.m_value.string->c_str()),
                j.m_value.string->size());
            break;
        ...
        case value_t::array: {
            ...
            if (use_type && !j.m_value.array->empty()) {
                ...
                std::vector<CharType> bjdx = { '[', '{', 'S', 'H', 'T', 'F',
                    'N', 'Z' }; // excluded markers in bjdata optimized type
            ...
            if (use_count) {
                oa->write_character(to_char_type('#'));
                write_number_with_ubjson_prefix(
                    j.m_value.array->size(), true, use_bjdata);
            }

            for (const auto& el : *j.m_value.array) {
                write_ubjson(
                    el, use_count, use_type, prefix_required, use_bjdata);
            }
        ...
        case value_t::binary: {
            ...
            if (use_count) {
                oa->write_character(to_char_type('#'));
                write_number_with_ubjson_prefix(
                    j.m_value.binary->size(), true, use_bjdata);
            }
        ...
        case value_t::object: {
            ...
            if (use_count) {
                oa->write_character(to_char_type('#'));
                write_number_with_ubjson_prefix(
                    j.m_value.object->size(), true, use_bjdata);
            }

            for (const auto& el : *j.m_value.object) {
                write_number_with_ubjson_prefix(
                    el.first.size(), true, use_bjdata);
                oa->write_characters(
  ...
  private:
    ...
    */
    void write_bson_binary(const string_t& name, const binary_t& value)
    {
        ...
        write_number<std::int32_t>(
            static_cast<std::int32_t>(value.size()), true);
    ...
    */
    CharType ubjson_prefix(
        const BasicJsonType& j, const bool use_bjdata) const noexcept
    {
        switch (j.type()) {
        case value_t::null:
            return 'Z';
    ...
    */
        ...
        const bool use_count, const bool use_type)
    {
        ...
        if (dtype == 'U' || dtype == 'C') {
            ...
            for (const auto& el : value.at(key)) {
                write_number(
                    static_cast<double>(el.m_value.number_float), true);
            }
        }
        return false;
    }
    ...
    */
    template <typename NumberType>
    void write_number(
        const NumberType n, const bool OutputIsLittleEndian = false)
    {
