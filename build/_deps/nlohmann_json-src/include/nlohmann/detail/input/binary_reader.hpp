  ...
  private:
                ...
                parse_error::create(113, chars_read,
                    exception_message(input_format_t::cbor,
                        concat("expected length specification (0x40-0x5B) or "
                               "indefinite binary array type (0x5F); last "
                               "byte: 0x",
                            last_token),
                        "binary"),
                    nullptr));
        ...
        if (len != 0) {
            ...
            if (len != static_cast<std::size_t>(-1)) {
                for (std::size_t i = 0; i < len; ++i) {
                    get();
                    if (JSON_HEDLEY_UNLIKELY(
                            !get_cbor_string(key) || !sax->key(key))) {
                        return false;
                    }
    ...
    */
    bool get_msgpack_object(const std::size_t len)
    {
        ...
        for (std::size_t i = 0; i < len; ++i) {
            get();
            if (JSON_HEDLEY_UNLIKELY(
                    !get_msgpack_string(key) || !sax->key(key))) {
                return false;
            }
    ...
    */
    bool get_ubjson_size_value(
        std::size_t& result, bool& is_ndarray, char_int_type prefix = 0)
    {
        if (prefix == 0) {
                    ...
                    if (result == 0
                        || result
                            == npos) // because dim elements shall not have
                                     ...
                                     // happened; it also can't be npos as it is
                                     // used to initialize size in
        ...
        if (current == '$') {
            ...
            if (JSON_HEDLEY_UNLIKELY(current != '#')) {
                ...
                return sax->parse_error(chars_read, last_token,
                    parse_error::create(112, chars_read,
                        exception_message(input_format,
                            concat("expected '#' after type information; last "
                                   "byte: 0x",
                                last_token),
                            "size"),
                        nullptr));
    ...
    bool get_ubjson_array()
    {
        ...
        if (input_format == input_format_t::bjdata
            ...
            auto it = std::lower_bound(bjd_types_map.begin(),
                bjd_types_map.end(), size_and_type.second,
            ...
            if (JSON_HEDLEY_UNLIKELY(it == bjd_types_map.end()
                ...
                return sax->parse_error(chars_read, last_token,
                    parse_error::create(112, chars_read,
                        exception_message(input_format,
                            "invalid byte: 0x" + last_token, "type"),
                        nullptr));
            }
    ...
    */
    bool get_ubjson_object()
    {
        ...
        if (size_and_type.first != npos) {
            ...
            if (size_and_type.second != 0) {
                for (std::size_t i = 0; i < size_and_type.first; ++i) {
                    if (JSON_HEDLEY_UNLIKELY(
                            !get_ubjson_string(key) || !sax->key(key))) {
                        return false;
                    }
                    if (JSON_HEDLEY_UNLIKELY(
                            !get_ubjson_value(size_and_type.second))) {
    ...
    bool get_ubjson_high_precision_number()
    {
        ...
        if (JSON_HEDLEY_UNLIKELY(
                result_remainder != token_type::end_of_input)) {
