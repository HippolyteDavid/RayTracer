    ...
    basic_json // NOLINT(cppcoreguidelines-special-member-functions,hicpp-special-member-functions)
{
  ...
  private:
    ...
    JSON_HEDLEY_RETURNS_NON_NULL static T* create(Args&&... args)
    {
        ...
        auto deleter
            = [&](T* obj) { AllocatorTraits::deallocate(alloc, obj, 1); };
...
#pragma warning(pop)
        ...
        if (!type_deduction) {
            ...
            if (JSON_HEDLEY_UNLIKELY(
                ...
                JSON_THROW(type_error::create(301,
                    "cannot create object from initializer list", nullptr));
            }
        }
  ...
  private:
    ...
    boolean_t get_impl(boolean_t* /*unused*/) const
    {
        ...
        JSON_THROW(type_error::create(302,
            detail::concat("type must be boolean, but is ", type_name()),
            this));
    }
    ...
    reference at(size_type idx)
    {
        ...
        if (JSON_HEDLEY_LIKELY(is_array())) {
            ...
            JSON_THROW(type_error::create(304,
                detail::concat("cannot use at() with ", type_name()), this));
        }
    }
  ...
  public:
        ...
        const ValueType& default_value) const
    {
        ...
        JSON_THROW(type_error::create(306,
            detail::concat("cannot use value() with ", type_name()), this));
    }
    ...
    IteratorType erase(IteratorType pos)
    {
        ...
        default:
            JSON_THROW(type_error::create(307,
                detail::concat("cannot use erase() with ", type_name()), this));
        }
    ...
    IteratorType erase(IteratorType first, IteratorType last)
    {
        // make sure iterator fits the current value
        if (JSON_HEDLEY_UNLIKELY(
                this != first.m_object || this != last.m_object)) {
            JSON_THROW(invalid_iterator::create(
  ...
  public:
    ...
    template <class... Args> std::pair<iterator, bool> emplace(Args&&... args)
    {
        ...
        if (JSON_HEDLEY_UNLIKELY(!(is_null() || is_object()))) {
            JSON_THROW(type_error::create(311,
                detail::concat("cannot use emplace() with ", type_name()),
                this));
        }
    ...
    iterator insert(const_iterator pos, const basic_json& val)
    {
        ...
        JSON_THROW(type_error::create(309,
            detail::concat("cannot use insert() with ", type_name()), this));
    }
        ...
        const_iterator pos, const_iterator first, const_iterator last)
    {
        ...
        if (JSON_HEDLEY_UNLIKELY(!is_array())) {
            JSON_THROW(type_error::create(309,
                detail::concat("cannot use insert() with ", type_name()),
                this));
        }
        ...
        const_iterator first, const_iterator last, bool merge_objects = false)
    {
        ...
        if (JSON_HEDLEY_UNLIKELY(!is_object())) {
            JSON_THROW(type_error::create(312,
                detail::concat("cannot use update() with ", type_name()),
                this));
        }
...
#define JSON_IMPLEMENT_OPERATOR(                                               \
    op, null_result, unordered_result, default_result)                         \
  ...
  private:
    bool compares_unordered(
        const_reference rhs, bool inverse = false) const noexcept
    {
...
#pragma GCC diagnostic pop
    ...
    JSON_HEDLEY_DEPRECATED_FOR(
        3.11.0, undef JSON_USE_LEGACY_DISCARDED_VALUE_COMPARISON)
    bool operator<=(const_reference rhs) const noexcept
    {
        if (compares_unordered(rhs, true)) {
            return false;
...
#undef JSON_IMPLEMENT_OPERATOR
    ...
    JSON_HEDLEY_WARN_UNUSED_RESULT static basic_json parse(IteratorType first,
        IteratorType last, const parser_callback_t cb = nullptr,
        const bool allow_exceptions = true, const bool ignore_comments = false)
    {
        basic_json result;
  ...
  public:
    ...
    static void to_ubjson(const basic_json& j,
        detail::output_adapter<std::uint8_t> o, const bool use_size = false,
        ...
        const bool strict = true, const bool allow_exceptions = true)
    {
        ...
        detail::json_sax_dom_parser<basic_json> sdp(result, allow_exceptions);
        auto ia = detail::input_adapter(std::forward<InputType>(i));
        const bool res = binary_reader<decltype(ia)>(
            std::move(ia), input_format_t::msgpack)
    ...
    void patch_inplace(const basic_json& json_patch)
    {
        ...
        const auto operation_remove = [this, &result](json_pointer& ptr) {
            ...
            if (parent.is_object()) {
                ...
                if (JSON_HEDLEY_LIKELY(it != parent.end())) {
                    ...
                    JSON_THROW(out_of_range::create(403,
                        detail::concat("key '", last_path, "' not found"),
                        this));
                }
        ...
        for (const auto& val : json_patch) {
            ...
            const auto get_value
                ...
                if (JSON_HEDLEY_UNLIKELY(
                        string_type && !it->second.is_string())) {
                    // NOLINTNEXTLINE(performance-inefficient-string-concatenation)
                    JSON_THROW(parse_error::create(105, 0,
        ...
        const std::string& path = "")
    {
        ...
        case value_t::array: {
            ...
            while (i < target.size()) {
                result.push_back(
                    { { "op", "add" }, { "path", detail::concat(path, "/-") },
                        { "value", target[i] } });
                ++i;
            }
        ...
        case value_t::object: {
            ...
            for (auto it = target.cbegin(); it != target.cend(); ++it) {
                if (source.find(it.key()) == source.end()) {
                    ...
                    const auto path_key
                        = detail::concat(path, '/', detail::escape(it.key()));
                    result.push_back({ { "op", "add" }, { "path", path_key },
                        { "value", it.value() } });
                }
            }
